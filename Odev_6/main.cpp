...
void fonk1(){
	...
	cout<<deger<< "Tane Cift Sayi Bulunuyor"<<endl;
...
//1 milyonun altýndaki her asal sayi 6k-1 veya 6k+1
...
void fonk2(){
	...
	for(int i = 1; i<=prob;++i){
		cout<<i<<". Problemin Puanini Giriniz:"<<endl;
		cin>>puan;
	    sorular[i] = puan;
	}
	dynamic2(prob);
	for(int i = 1;i<=prob;++i){ if(kontrol(i)){toplamPuan+=sorular[i];} }
